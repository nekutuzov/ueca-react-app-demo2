...
type FileSelectorParams = BaseParams<FileSelectorStruct>;
type FileSelectorModel = BaseModel<FileSelectorStruct>;

function useFileSelector(params?: FileSelectorParams): FileSelectorModel {
    ...
    async function _selectFiles(fileMask: string, multiselect: boolean): Promise<File[]> {
        const promise = new Promise<File[]>((resolve) => {
            ...
            setTimeout(() => {
                ...
                // (the native file input only fires when the selection differs from last time)
                input.value = "";
                input.focus();
                input.click();
            });
...
export { FileSelectorModel, FileSelectorParams, useFileSelector, FileSelector }
