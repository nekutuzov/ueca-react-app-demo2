...
describe("OtherLayout", () => {
    describe("lookupRoute", () => {
        it("resolves each registered external address to its own route", async () => {
            ...
            const addresses = Object.keys(otherRoutes);
        ...
        // Regression: router.tsx built the pattern from the parsed URL, whose pathname for an
        // origin-only address is "/", so it demanded a trailing slash the registered key does not have.
        // A GoToRoute to "https://cranesoft.net" was claimed by neither layout.
        it("resolves an origin-only address by its own key", async () => {
            const { model } = await mount(OtherLayout, { id: "otherLayout" });
        ...
        // Regression: router.tsx built every absolute pattern as protocol + "//" + host + path, and
        // dropped the first path segment as the empty one before a leading slash. A mailto address has
        // no "//" and its whole address is that first segment, so the pattern was /^mailto:\/\/(?:\?|$)/.
        it("resolves a mailto address by its own key", async () => {
            const { model } = await mount(OtherLayout, { id: "otherLayout" });

            expect(model.lookupRoute("mailto:cranesoft@protonmail.com")?.path).toBe("mailto:cranesoft@protonmail.com");
            expect(model.lookupRoute("mailto:someone@protonmail.com")).toBeUndefined();
        });
    });
